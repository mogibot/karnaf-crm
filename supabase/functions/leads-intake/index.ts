...
import { persistWebhookInbox, finalizeWebhookInbox } from '../_shared/webhook-inbox.ts';
...
Deno.serve(async (req) => {
  ...
  // Fail-closed: production must have INTAKE_WEBHOOK_SECRET set. Dev/local
  // can opt out via WEBHOOK_ALLOW_UNSIGNED=true.
  ...
  // Persist the raw payload + safe headers BEFORE any business logic, so a
  // crash mid-handler still leaves a replay-able row in webhook_inbox.
  const inboxId = await persistWebhookInbox(supabase, {
    source: 'leads-intake', req, rawBody, correlationId,
  });

  let payload: Record<string, unknown>;
  try { payload = JSON.parse(rawBody); } catch {
    await finalizeWebhookInbox(supabase, inboxId, 'client_error', 'invalid_json');
  ...
  try {
    ...
    if (!phone && !email) {
      await finalizeWebhookInbox(supabase, inboxId, 'client_error', 'missing_phone_or_email');
    ...
    await finalizeWebhookInbox(supabase, inboxId, 'server_error', msg);
    log.error('lead_intake_exception', { fn: 'leads-intake', correlationId, err: msg });
    return jsonResponse(req, { error: 'Internal error', detail: msg.slice(0, 200) }, 500);
  }
});
