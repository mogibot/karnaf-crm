...
-- runs, so an admin can replay it later via the WebhookReplayAdminPage.
--
-- Scope:
--   * Inserted by all webhook functions immediately after auth passes,
--     before any DB writes. (See _shared/webhook-inbox.ts.)
...
create table if not exists webhook_inbox (
  ...
  source text not null,            -- 'leads-intake' | 'whatsapp' | 'ig' | 'fb-leadgen' | 'email' | 'payment' | 'provider-status'
  -- Headers as a jsonb object so signature replay matches exactly. We
  -- strip the auth bearer before persisting (see helper); the HMAC sig
  -- header stays so replays still validate.
  headers_json jsonb not null,
  body text not null,              -- raw text, exactly as fetched
  correlation_id text,
  received_at timestamptz not null default now(),
  -- Processing outcome. Null = pending replay (handler still running).
  ...
  error_message text,
  -- When set, this row was replayed FROM an earlier row; lets the UI
  -- show the replay chain without polluting the original.
...
-- Retention: purge rows older than 30 days. Wire from nightly-jobs.
...
-- No RLS — service-role only. (No `enable row level security` line.)
